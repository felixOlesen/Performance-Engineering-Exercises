...
def validate_args():
    if not args.platform or (
        args.platform.lower() != "h" and args.platform.lower() != "l"
    ):
        print(
            Fore.RED + "[Warning] Please provide a valid platform to find the problem"
        )
        return False

    if not args.filename:
        print(
            Fore.RED + "[Warning] Please provide a valid filename to find the problem"
        )
        return False

    if not find_file_modern(args.filename):
        print(Fore.RED + "File Not Found: " + args.filename)
        return False
...
def main():
    ...
    process = subprocess.run(
        ["python", platform_dict[args.platform.lower()] + "/" + args.filename],
        capture_output=True,
        text=True,
    )
    ...
    if process.returncode == 0:
        ...
        print(Fore.GREEN + "----- Process Failed -----")
